...
class MazeRenderer:
    ...
    def draw_player_run(self, player_pos, move_history, corrected_path):
        for r, c in move_history:
            self.draw_cell(r, c, C_PLAYER_TRACE, shrink=3)

        for r, c in corrected_path:
            self.draw_cell(r, c, C_PLAYER_CORRECT, shrink=5)

        if player_pos is not None:
            pr, pc = player_pos
            self.draw_cell(pr, pc, C_PLAYER)
            lbl = self.font_xs.render("P", True, (255, 255, 255))
            self.screen.blit(
                lbl,
                (self.maze_x_offset + pc * self.cell_size + 4, self.maze_y_offset + pr * self.cell_size + 3),
            )
    ...
    def draw_legend(self):
        ...
        color_map = {
            ...
            "goal": C_GOAL,
            "player": C_PLAYER,
            "player_trace": C_PLAYER_TRACE,
            "player_correct": C_PLAYER_CORRECT,
            "dfs_path": C_DFS_PATH,
            "bfs_path": C_BFS_PATH,
